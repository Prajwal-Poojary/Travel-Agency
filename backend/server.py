...
def get_virtual_tours(search: Optional[str] = None, country: Optional[str] = None, tour_type: Optional[str] = None, featured_only: Optional[bool] = None, limit: int = 50, skip: int = 0, response: Response = Response()):
    limit = max(1, min(limit, 100))
    skip = max(0, skip)
    query = {'active': True}
    if search:
